...
module Board : sig 
  ...
  let inRange {x;y} = x>=0 && y>=0 && x<size && y<size
  ...
  let make () = Array.make_matrix size size None
...
module GameState : sig 
  ...
  let isWinning {step;_} = Int.equal step (Board.size * Board.size)
  ...
  let isValidTarget board target =
    Board.inRange target && Option.is_none (Board.get board target)
