...
let make_table () =
  ...
  |> Table_logger.add_column ~label:"Nodes Est" ~extract_and_format:(fun w r -> Printf.sprintf "%*s" w (format_number r.nodes_est))
  |> Table_logger.add_column ~label:"Fails Est" ~extract_and_format:(fun w r -> Printf.sprintf "%*s" w (format_number r.fails_est))
  ...
  |> Table_logger.add_column ~label:"%Done" ~extract_and_format:(fun w r -> Printf.sprintf "%*s" w (format_number r.progress_pct))
