...
let () =
  ...
  Stochastic_progress.run_with_progress ~batch_size ~on_progress:(fun p ->
    ...
    Printf.printf "%-5d | %-7d | %12s | %12s | %-8s | %-6d | %12d | %10d | %10d | %5.1f%% | %-12s | %-12s | %-10s\n%!"
      ...
      net_nodes
      undersampled_pct
      (format_number p.progress_percent)
